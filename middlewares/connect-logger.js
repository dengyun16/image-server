...
function getLogger(logger4js, options) {
	...
	return function (req, res, next) {
		...
		res.locals.ip = req.headers['x-forwarded-for'] || req.ip || req._remoteAddress ||
		...
		function log(event) {
			...
			if (thislogger.isLevelEnabled(level)) {
				...
				if (typeof fmt === 'function') {
					...
					thislogger.log(level, format(fmt, combined_tokens) + '\t' + event);
				}
			}
		}
		...
		if (thislogger.isLevelEnabled(level) || options.level === 'auto') {
			...
			res.on('finish', logEvent('finish'));
			res.on('close', logEvent('close'));
		}
...
function assemble_tokens(req, res, custom_tokens) {
	...
	default_tokens.push({
		token      : ':remote-addr',
		replacement: req.headers['x-forwarded-for'] || req.ip || req._remoteAddress || res.locals.ip ||
		(req.socket && (req.socket.remoteAddress || (req.socket.socket && req.socket.socket.remoteAddress)))
	});
	default_tokens.push({
		token      : ':user-agent',
